...
\section{MAST analysis}\label{mast-analysis}
...
The EDF scheduling is dynamic-priority algorithm that assigns priorities to individual jobs of a task based on its absolute deadline.
In particular, the earlier the deadline, the higher the priority.
An absolute deadline of a job is computed as its release event plus its relative deadline. 
Such relative deadline is a static attribute, missing in FPS algorithm, and corresponding to the maximum response time allowed to each job's task.
%A job release causes the ready queue's re-ordering in which the job's position is inversely proportional to it's absolute deadline.
%EDF is a dynamic-priority algorithm, indeed it assigns different priorities to the individual jobs.

The runtime adopted is an Ada Ravenscar runtime variant implementing an EDF scheduling coupled with Deadline Floor Protocol (DFP) locking.
DFP \cite{resource-sharing} is the EDF counterpart of IPCP, outlined in section 1.1.
Indeed, rather than assigning a ceiling priority to each shared resource, a deadline floor is computed and rather than raising the priority of a task
to the resource's ceiling, the task reduces its current deadline to reflect the floor value of the resource.
...
\subsection{MAST Restrictions}

First, as mentioned in §\ref{mast-analysis}, MAST demands further restrictions for EDF analysis tools.\newline
To keep consistency between our application and its model, we had to discard Offset-based and Holistic analisys tools:
the former rollback to the latter, the latter lacks shared resources support.\newline
The last standing option was EDF Monoprocessor tool which implements the exact response time analysis for single-processor EDF systems.
...
%cos'è SRP
Stack Resource Policy (SRP) \cite{resource-sharing} is the synchronization protocol required by the EDF Monoprocessor tool.
Under SRP each job is assigned a preemption level inversely proportional to the task relative deadline.
In turn, each resource is assigned a ceiling preemption level defined as the maximum preemption level of any job that may access it.
Defining the system ceiling as the highest ceiling of all the resources which are held by some job at any time t, allow the definition of SRP locking policy:
a job can acquire a given resource if and only if its preemption level is higher than the system ceiling.
...
the worst-case behavior equivalence among SRP and DFP.
Indeed they lead to the same blocking time in the scheduling analysis \cite{resource-sharing}.

%MAST
To write down a suitable MAST model, the preemption level for each Task must be set according to SRP.
An available assignment is the one in Figure \ref{tab:preemption-levels}.
...
Based on the previous step, Figure \ref{tab:ceiling-preemption-level} show a sound definition of resource ceilings.
...
EDF Monoprocessor tool is based on the analysis developed by Spuri \cite{spuri}.\newline
As depicted in Figure \ref{fig:spuri}, the worst case response time (WCRT) of a task $i$ is found in a busy period $[t_{1},t_{2}]$
in which all other tasks are released synchronously at $t=0$ and then at their maximum rate.
Such busy period is characterized by $i$'s instance released at time $t=a$, $a>=0$, possibly preceded by other instances of
task $i$.
Note: $t_{2}$ it's the completion time of $i$.
...
With this being said, the transactions of this model are exactly those in the FPS model with independent task set as 
outlined in §\ref{independent-tasks}.\newline
Hence, four transaction each one composed of a single activity.\newline
Feeding EDF Monoprocessor tool with a model entailing the highest small\_Whetstone values reached by Offset-based analysis,
...
synchronously leads to a great pessimism in worst case response time.
Figure \ref{fig:timeline-WCRT-RP} approximate the worst arrival pattern causing Regular\_Producer's WCRT.
Taking advantage of dependency, RP will never compete for the cpu with ALR because the completion of the former
causes the release event of latter.
...
%feasible scheduling?\newline
Since Rate Monotonic is the FPS counterpart between exact response time analysis
for single-processor, it seems reasonable to take it as a touchstone.
Indeed, EDF Monoprocessor results depected in figure \ref{tab:EDF-mono-rate-mono} 
are close to the one obtained with Rate Monotonic, under identical load.\newline
%why?
%Although the specific critical instant differs between Rate Monotonic and EDF Monoprocessor analisys, both are 

%side note
We believe that the ephemeral slack surplus experienced is due to the absence of the system ticker in EDF analysis.
...
EDF is an optimal scheduler and can handle a total utilization up to 1.
In advance, any feasible pre-emptive schedule can be easily transformed into an EDF pre-emptive schedule
without affecting its feasibility. % \cite{}.
Hence, we expect the EDF runtime to perform at least as well as the FPS counterpart in terms of utilisation handled.

\begin{figure}[!htbp]
   \centering
   \includegraphics[width=6in]{images/timeline-edf-offsets}
   \caption{GEE execution under EDF scheduling with maximum utilisation}
   \label{fig:timeline-edf-offsets}
\end{figure}

Figure \ref{fig:timeline-edf-offsets} exhibits the schedule produced by EDF up to 
the hyperperiod $H=3$. Total utilisation of 83.28\%, tested against FPS scheduling
in §\ref{maximum-utilisation}, is given by Whetstone values in Table \ref{tab:Whet-max-util}
...
\begin{table}[!htbp]
   ...
   \begin{tabular}{lllcl}
    ...
    \toprule
   \end{tabular}
   \caption{Whetstone values leading to maximum utilisation}
   \label{tab:Whet-max-util}
 \end{table}

Despite EDF optimality, observing the aforementioned timeline, 
there is no room for utilisation increasing:
RP already runs at zero laxity, OCP and ALR cannot exceed an execution time of 0.5 seconds
without causing a fatal release jitter to RP itself.\newline
As a side consideration, both EDF and FPS algorithm under those timing constraints lead to the 
same schedule. Since FPS's priorities comply a deadline monotonic assignment and no job with 
a longer relative deadline is released before a job subject to a shorter one, consistency
between those schedules is guaranteed.
%Note that the above observation applys only for a not overload system.
...
%As pointed out in §\ref{}, with a precedence relation between RP and OCP a concurrent
%access to request\_buffer resource it's not reasonable.
...
We have observed this behaviour in our tests by increasing the RP worload of a small amount $\epsilon = 0.02$, reaching a WCET of approximately 0.52s. The hyperperiod $H$ considered must also be rescaled considering the ration between the overload utilisation $H^*$ and the previous utilisation $U$:

$H^* = H * U*/U = 3 * ({0.52 \over 1} + {0.5 \over 3} + {0.5 \over 3})*{5 \over 6} \approx 3.07s$.
